...
export class ViolationReportComponent implements OnInit {
  ...
  async submit() {
    ...
    if (error) {
      ...
      alert('Failed to submit violation report: ' + error.message);
    } else {
      console.log('Violation report submitted successfully');
      this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Violation report submitted successfully!' });
