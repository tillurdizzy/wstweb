...
import { ToastModule } from 'primeng/toast'; // Replaced DialogModule
...
export class ArchRequestComponent implements OnInit {
  ...
  async submit() {
    ...
    if (error) {
      console.error('Error submitting architectural request:', error.message);
      alert('Failed to submit architectural request: ' + error.message);
    } else {
      console.log('Architectural request submitted successfully');
      this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Architectural request submitted successfully!' });
